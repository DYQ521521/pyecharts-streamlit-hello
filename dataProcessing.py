...
class GetDF:
    ...
    def getClassStuCountDF(self):  # 获取班级及班级人数的数据
        ...
        dfClass = pd.DataFrame({'班级': dfClass.index, '人数': dfClass.values}).sort_values(by='班级',
                                                                                        ascending=True)  #
        # 用获取的Series的 index和value重新构造dataframe  # 用班级列进行排序
        dfClass['班级'] = dfClass['班级'].astype(str)  # 将班级列转化为str字符串
        dfClass['班级'] = dfClass['班级'] + '班'  # 在班级列的名称后面加"班"成为x班
        return dfClass

    def getTheStuResults(self, stuName, courseList=basicData.coures6List):  # 获取指定学生各科成绩
    ...
    def getAllCourseRanking(self, courseList=basicData.coures6List):  # 获取所有成绩和所有学科排名 # 班级 姓名 各科分数 各科排名，按总分进行排名的df
    ...
    def getStuAllRank(self, stuName, rankList=basicData.ranking6List):  # 获取各科排名的数值
    ...
    def classAvgAndVariance(self, choose=False, courseList=basicData.coures6List):  # 获得 班级 全部学科的均分 全部学科的方差
